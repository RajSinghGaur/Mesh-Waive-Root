...
char message[] = "{\"device\":\"%s\",\"epoch\":\"%ld\",\"is_root\":\"%d\"}";
char dev_name[] = "N1";
                        ...
                        int32_t event_id, void *event_data)
{
    ...
    case MESH_EVENT_STARTED:
    {
        ...
        xTaskCreate(root_chk, "ROOT_CHK", 3072, NULL, 5, NULL);
    }
    break;
    ...
    {
        mesh_event_routing_table_change_t *routing_table = (mesh_event_routing_table_change_t *)event_data;
        ESP_LOGW(MESH_TAG, "<MESH_EVENT_ROUTING_TABLE_REMOVE>remove %d, new:%d, layer:%d", routing_table->rt_size_change, routing_table->rt_size_new, mesh_layer);
    }
    break;
    ...
    case MESH_EVENT_PARENT_CONNECTED:
    {
        ...
        ESP_LOGI(MESH_TAG, "<MESH_EVENT_PARENT_CONNECTED>layer:%d-->%d, parent:" MACSTR "%s, ID:" MACSTR ", duty:%d", last_layer, mesh_layer, MAC2STR(mesh_parent_addr.addr), esp_mesh_is_root() ? "<ROOT>" : (mesh_layer == 2) ? "<layer2>"
                                                                                                                                                                                                                                : "",
                      ...
                      int32_t event_id, void *event_data)
{
    ...
    if (esp_mesh_is_root())
    {
        xTaskCreate(tmps_waive, "WAIVE_CHK", 3072, NULL, 5, NULL);
    }
}

void app_main(void)
{
    ESP_LOGI("[MAIN]", "START, FREE MEMORY : %d | MIN FREE HEAP MEMORY %d", esp_get_free_heap_size(), esp_get_minimum_free_heap_size());
    ...
    ESP_LOGI("[MAIN]", "END, FREE MEMORY : %d | MIN FREE HEAP MEMORY %d", esp_get_free_heap_size(), esp_get_minimum_free_heap_size());
}
